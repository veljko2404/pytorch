...
I trained a ResNet50 model on CIFAR-10 with cross-entropy loss (no distillation). Training lasted 300 epochs with an initial learning rate of 0.1, decaying by following a cosine curve to 0. The teacher achieved a test
accuracy of 90.1\% in 1h 56m 17s. This ResNet50 serves as the fixed teacher for subsequent distillation.
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{2.png}
    \caption{Knowledge Distillation}
    \label{fig:self_distillation}
\end{figure}

\subsection{ResNet18 Student (Traditional KD)}
I used a ResNet18 as the student model. During distillation, the student is trained with a combined loss (\ref{eq:total_kd_loss}): the usual cross-entropy loss on the true labels (hard targets) plus a distillation loss between the student’s and teacher’s output distributions. Specifically, I applied a softened softmax (\ref{eq:softmax}) (with temperature $T$) to the teacher’s logits to produce a high-entropy “soft target” distribution. The student’s output (at the same temperature) is trained to match this distribution via KL-divergence (\ref{eq:kd_loss}). In practice we followed standard KD settings: we set $T=4$ and used a weighting factor so that the KL loss and the hard-label cross-entropy each contribute appropriately. The student was trained for the same number of epochs (200) with identical learning rate schedule. This distillation process took 2h 13m 6s and resulted in 94.2\% test accuracy.
accuracy of 90.1\% in 1h 56m 17s. This ResNet50 serves as the fixed teacher for subsequent distillation.
\begin{equation}
\label{eq:softmax}
\text{Softmax with temperature:}\quad
...
\text{Total distillation loss:}\quad
...
I implemented the self-distillation framework from Zhang on the same ResNet18 architecture. Following “Be Your Own Teacher”, I divided ResNet18 into 4 segments. We attached auxiliary classifiers (fully-connected layers) at the end of each segment, so there are four classifiers in total. During training, the deepest (4th) classifier’s logits provide the pseudo-label distribution as defined in Eq.~(\ref{eq:self_teacher}). Each shallower classifier is trained to match the deepest classifier’s soft outputs (\ref{eq:self_student}) (using a KL-divergence loss (\ref{eq:self_kd_loss})) in addition to its own cross-entropy loss on the true labels. The overall training objective is given by Eq.~(\ref{eq:self_total_loss}). This effectively distills knowledge from layer 4 into layers 1–3. After training, only the original ResNet18 (up to layer 4) is retained for evaluation; the auxiliary classifiers are discarded. This self-distillation training took 2h 13m 52s and achieved 94.4\% accuracy.
\begin{equation}
\label{eq:self_teacher}
\text{Teacher soft predictions:}\quad
...
\text{Student soft preds at layer k}\quad
...
\text{Self-distillation loss:}\quad
...
The improvement of self-distillation is supported by theoretical arguments. These factors contribute to better generalization and effective convergence, explaining the observed accuracy gain. Importantly, self-distillation achieves this without requiring a separate large teacher network.
...
\bibitem{b4} What is Knowledge Distillation? A Deep Dive.
\newblock \\\url{https://blog.roboflow.com/what-is-knowledge-distillation/}

\bibitem{b5} A Primer on Knowledge Distillation in NLP — Part 1
\newblock \\\url{https://medium.com/analytics-vidhya/a-primer-on-knowledge-distillation-in-nlp-part-1-754ac00dadc6}
